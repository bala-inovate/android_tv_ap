...
if  [[ $? -eq 0 ]]; then
echo $i >> ./s.txt  &&  echo "The video content is moved successfully to this device $i ." | mail -s "Tamil tv content movement of $thismonth month" balaji.it@vcaregroup.in
echo "done"
fi
}
...
for j in $(cat ./log.txt | awk '{ print $1 }')
do	 
ping -4 -c4 $j >> /dev/null
if [ $? -eq 0 ]; then

aa=$( cat /root/ai/s.txt | grep -w $j )
